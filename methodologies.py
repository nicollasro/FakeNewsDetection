...
def esfera ():
        ...
        for index, row in df2.iterrows():
                if row['Total']>=intervalo_ver_neg and row['Total']<=intervalo_ver_pos:
        ...
        ttest,pval = ttest_ind(df2['Total'].loc[df["probability"] == 1], df2['Total'].loc[df["probability"] == -1])
        if pval<0.05:
                print('rejecting null hypothesis')
        else:
                print('accepting null hypothesis')
        '''
        #print(df2['Total'].loc[df["probability"] == 1].shape())
        #print(df2['Total'].loc[df["probability"] == -1].shape())
        #print(df2['Total'].shape())
        f=plt.figure(3)
        ...
        lista=[]
        for index, row in df2.iterrows():
                lista.append(row['Total'])
        lista.sort()
        print('Primeiro')
        print (lista[0])
        print ('Ultimo')
        print (lista[-1])
        intervalos=(lista[1]-lista[0])/((int(sqrt(30809+3180)))-1)
        limites=[lista[0]]
        probabilidades=[]
        for i in range(((int(sqrt(30809+3180)))-1)):
                limites.append(lista[0]+intervalos)
        print ('TAMANHO DO LIMITE:'+str(len(limites)))
        for index, row in df2.iterrows():
                for i in range(len(limites)):
                        if row['Total']>limites[i]:
                                r_menos=limites[i] #Acha o limite inferior
                                r_mais=limites[i+1] #Acha o limite superior
                                ...
                                #print (prob_menos)
                                #print (prob_mais)
                                prob_final=abs(prob_mais-prob_menos)
        ...
        for thresh in thresholds:
                ...
                for i in range(len(probabilidades)):
                        #print(df['probability'].loc[i])
                        if probabilidades[i]> thresh:
                                if df['probability'].loc[i] == 1:
                                        TP=TP+1
                                if df['probability'].loc[i] == -1:
                                        FP=FP+1
                print(TP)
